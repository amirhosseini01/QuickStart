...
public class PaginatedList<T> : List<T>
{
    ...
    public static async Task<PaginatedList<T>> CreateAsync(IQueryable<T> source, PaginatedListFilter filter, CancellationToken ct = default)
    {
        var count = await source.CountAsync();
        var items = await source.Skip((filter.PageIndex - 1) * filter.TakeSize).Take(filter.TakeSize).ToListAsync(ct);
